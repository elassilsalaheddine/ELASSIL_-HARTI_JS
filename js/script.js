...
function animationLoop() {
    ...
    switch (gameState) {
        ...
        case "EcranChangementNiveau":
                afficheEcranChangementNiveau(ctx);
            break;
        case 'gameOver':
            afficheGameOver(ctx);
            break;
...
function afficheMenuStart(ctx) {
  ...
  mrvoiture.draw(ctx);
  mrvoiture.move();
  traiteCollisionsMrvoitureAvecBords();
  ctx.translate(80, 200);
  ctx.fillStyle = "#0f056b";
  ctx.font = "80pt Shakerato-PERSONAL";
  ctx.fillText("Sby GAME", 80, 45);
  ctx.fillStyle = "black";
  ctx.font = "50pt Shakerato-PERSONAL";
  ctx.fillText("START ", 220, 160);
  ctx.fillStyle = "red";
  ctx.font = "20pt Shakerato-PERSONAL";
  ctx.fillText("Press Space to Start", 265, 200);
  
    
    ctx.strokeText("Press space to start", 90, 350);
    if (inputState.space) {
...
let mrvoiture = {
    ...
    draw: function (ctx) {
        ctx.save();
        ctx.translate(0, 0);
        ctx.drawImage(assets.mrvoiture, this.x, this.y, this.l, this.h);
        ctx.restore();
    }
    ,
    move: function () {
...
function traiteCollisionsMrvoitureAvecBords() {
    if (mrvoiture.x > canvas.width - mrvoiture.l - 125) {
      mrvoiture.x = canvas.width - mrvoiture.l - 125;
      mrvoiture.vitesseX = -mrvoiture.vitesseX;
    } else if (mrvoiture.x < 125) {
      mrvoiture.x = 125;
    mrvoiture.vitesseX = -mrvoiture.vitesseX;
    }
    if (mrvoiture.y < 0) {
      mrvoiture.y = 0;
      mrvoiture.vitesseY = -mrvoiture.vitesseY;
    } else if (mrvoiture.y + mrvoiture.h > canvas.height) {
      mrvoiture.y = canvas.height - mrvoiture.h;
      mrvoiture.vitesseY = -mrvoiture.vitesseY;
    }
  }
